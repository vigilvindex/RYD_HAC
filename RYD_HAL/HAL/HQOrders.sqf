private ["_HQ","_nObj","_Trg","_vHQ","_landE","_dstMin","_dstAct","_dstMin","_PosObj1","_ReconAv","_onlyL","_unitvar","_busy","_vehready","_solready","_effective","_ammo","_Gdamage",
	"_nominal","_current","_veh","_forRRes","_RRp","_AttackAv","_FlankAv","_exhausted","_inD","_ResC","_stages","_rcheckArr","_gps","_LMCUA","_reserve","_recvar","_resting",
	"_deployed","_capturing","_reconthreat","_FOthreat","_snipersthreat","_ATinfthreat","_AAinfthreat","_Infthreat","_Artthreat","_HArmorthreat","_LArmorthreat","_Carsthreat",
	...
	"_isAttacked","_captCount","_captLimit","_forCapt","_groupCount","_LMCU","_WADone","_WAchance","_armored","_WAAv","_where","_heldBy","_howMuch"];
...
if ((count _landE) > 0) then 
	...
	_Trg = leader (_HQ getVariable ["RydHQ_NearestE",grpNull]);
	};
	
_PosObj1 = position (vehicle _Trg);
...
foreach (((_HQ getVariable ["RydHQ_RAirG",[]]) + (_HQ getVariable ["RydHQ_ReconG",[]]) + (_HQ getVariable ["RydHQ_FOG",[]]) + (_HQ getVariable ["RydHQ_SnipersG",[]]) + (_HQ getVariable ["RydHQ_NCrewInfG",[]]) - ((_HQ getVariable ["RydHQ_SupportG",[]]) + (_HQ getVariable ["RydHQ_NCCargoG",[]])) + _onlyL) - ((_HQ getVariable ["RydHQ_AOnly",[]]) + (_HQ getVariable ["RydHQ_CargoOnly",[]])));
...
_exhausted = _HQ getVariable ["RydHQ_Exhausted",[]];
	...
	if not (isNull _x) then
		...
		if (((_HQ getVariable ["RydHQ_Withdraw",1]) > 0) and not (_x in ((_HQ getVariable ["RydHQ_SpecForG",[]]) + (_HQ getVariable ["RydHQ_SnipersG",[]])))) then
			...
			if (not (_x in _exhausted) and ((random (2 + (_HQ getVariable ["RydHQ_Recklessness",0.5]))) < (_inD * (_HQ getVariable ["RydHQ_Withdraw",1])))) then {_exhausted set [(count _exhausted),_x]}; 
			};
		}
	}
foreach (((_HQ getVariable ["RydHQ_Friends",[]]) - ((_HQ getVariable ["RydHQ_reconG",[]]) + (_HQ getVariable ["RydHQ_FOG",[]]) + ((_HQ getVariable ["RydHQ_NCCargoG",[]]) - (_HQ getVariable ["RydHQ_NCrewInfG",[]])) + (_HQ getVariable ["RydHQ_SupportG",[]]))) - (_HQ getVariable ["RydHQ_ROnly",[]]));
...
_FlankAv = _FlankAv - (_HQ getVariable ["RydHQ_NoFlank",[]]);
		...
		if (_HQ getVariable ["RydHQ_FlankReady",false]) then
			...
			if ((count (_HQ getVariable ["RydHQ_KnEnemies",[]])) > 0) then
				...
				if not (_HQ getVariable ["RydHQ_DefDone",false]) then
					{
					_gap = (time - _timeStamp) - (60 * (1 + ((vehicle (leader _HQ)) distance (position (_HQ getVariable ["RydHQ_Obj",vehicle (leader _HQ)])))/1000));
					if (_gap > 0) then
						{
...
_rcheckArr = [(_HQ getVariable ["RydHQ_Garrison",[]]),_ReconAv,_FlankAv,(_HQ getVariable ["RydHQ_AOnly",[]]),_exhausted,(_HQ getVariable ["RydHQ_NCCargoG",[]]),_Trg,(_HQ getVariable ["RydHQ_NCVeh",[]])];
...
if (((_HQ getVariable ["RydHQ_NoRec",1]) * ((_HQ getVariable ["RydHQ_Recklessness",0.5]) + 0.01)) < (random 100)) then 
	...
	if ((count (_HQ getVariable ["RydHQ_KnEnemiesG",[]])) == 0) then
		{
		if (not ((count (_HQ getVariable ["RydHQ_RAirG",[]])) == 0) and ((count (_HQ getVariable ["RydHQ_ReconAv",[]])) > 0) and not (_HQ getVariable ["RydHQ_ReconDone",false]) and not ((_HQ getVariable ["RydHQ_ReconStage",1]) > _stages)) then
			{
			_gps = [(_HQ getVariable ["RydHQ_RAirG",[]]),"R",_rcheckArr,20000,true] call RYD_Recon;

				{
				if ((_HQ getVariable ["RydHQ_ReconStage",1]) > _stages) exitWith {};
				...
				_HQ setVariable ["RydHQ_ReconAv",_reconAv];
				[_x,_PosObj1,(_HQ getVariable ["RydHQ_ReconStage",1]),_HQ] spawn HAL_GoRecon;
				}
			foreach _gps
			};

		if (not ((count (_HQ getVariable ["RydHQ_reconG",[]])) == 0) and ((count (_HQ getVariable ["RydHQ_ReconAv",[]])) > 0) and not (_HQ getVariable ["RydHQ_ReconDone",false]) and not ((_HQ getVariable ["RydHQ_ReconStage",1]) > _stages)) then
			{
		...
		if (not ((count (_HQ getVariable ["RydHQ_FOG",[]])) == 0) and ((count (_HQ getVariable ["RydHQ_ReconAv",[]])) > 0) and not (_HQ getVariable ["RydHQ_ReconDone",false]) and not ((_HQ getVariable ["RydHQ_ReconStage",1]) > _stages)) then
			{
		...
		if (not ((count (_HQ getVariable ["RydHQ_snipersG",[]])) == 0) and ((count (_HQ getVariable ["RydHQ_ReconAv",[]])) > 0) and not (_HQ getVariable ["RydHQ_ReconDone",false]) and not ((_HQ getVariable ["RydHQ_ReconStage",1]) > _stages)) then
			{
		...
		_onlyL = (_HQ getVariable ["RydHQ_LArmorG",[]]) - (_HQ getVariable ["RydHQ_MArmorG",[]]);
		if (not ((count _onlyL) == 0) and ((count (_HQ getVariable ["RydHQ_ReconAv",[]])) > 0) and not (_HQ getVariable ["RydHQ_ReconDone",false]) and not ((_HQ getVariable ["RydHQ_ReconStage",1]) > _stages)) then
			{
		...
		if (not ((count ((_HQ getVariable ["RydHQ_NCrewInfG",[]]) - (_HQ getVariable ["RydHQ_SpecForG",[]]))) == 0) and ((count (_HQ getVariable ["RydHQ_ReconAv",[]])) > 0) and not (_HQ getVariable ["RydHQ_ReconDone",false]) and not ((_HQ getVariable ["RydHQ_ReconStage",1]) > _stages)) then
			{
		...
		_LMCUA = (_HQ getVariable ["RydHQ_Friends",[]]) - ((_HQ getVariable ["RydHQ_NavalG",[]]) + (_HQ getVariable ["RydHQ_StaticG",[]]) + (_HQ getVariable ["RydHQ_SupportG",[]]) + (_HQ getVariable ["RydHQ_ArtG",[]]) + (_HQ getVariable ["RydHQ_AOnly",[]]) + (_HQ getVariable ["RydHQ_SpecForG",[]]) + (_HQ getVariable ["RydHQ_CargoOnly",[]]));
		if (not ((count _LMCUA) == 0) and not (_HQ getVariable ["RydHQ_ReconDone",false]) and not ((_HQ getVariable ["RydHQ_ReconStage",1]) > _stages)) then
			{
...
if (not ((_HQ getVariable ["RydHQ_ReconDone",false])) and ((count (_HQ getVariable ["RydHQ_KnEnemies",[]])) == 0)) exitwith 
	...
	if (_HQ getVariable ["RydHQ_Orderfirst",true]) then 
		...
		if not (_resting) then {[_x,_HQ] spawn HAL_GoRest }
		}
	...
	if (_HQ getVariable ["RydHQ_IdleOrd",true]) then
		{
			...
			if (not (_busy) and ((count (waypoints _x)) <= 1) and not (_deployed) and not (_capturing) and (not (_x in (_HQ getVariable ["RydHQ_NCCargoG",[]])) or ((count (units _x)) > 1))) then {deleteWaypoint ((waypoints _x) select 0);[_x,_HQ] spawn HAL_GoIdle }
			}
		foreach _reserve;
		}
	};
...
foreach ([(_HQ getVariable ["RydHQ_Obj1",(leader _HQ)]),(_HQ getVariable ["RydHQ_Obj2",(leader _HQ)]),(_HQ getVariable ["RydHQ_Obj3",(leader _HQ)]),(_HQ getVariable ["RydHQ_Obj4",(leader _HQ)])] - [(_HQ getVariable ["RydHQ_Obj",(leader _HQ)])]);
...
if ((_isAttacked <= 3) or (_captCount < _captLimit)) then
	{	
	if ((not ((_HQ getVariable ["RydHQ_NObj",1]) > 4) and ((random 100) > ((count (_HQ getVariable ["RydHQ_KnEnemies",[]]))*(5/(0.5 + (2*(_HQ getVariable ["RydHQ_Recklessness",0.5])))))) and not 
		...
		{
		_checked = [];
		_forCapt = (_HQ getVariable ["RydHQ_NCrewInfG",[]]) - ((_HQ getVariable ["RydHQ_SupportG",[]]) + (_HQ getVariable ["RydHQ_SpecForG",[]]) + (_HQ getVariable ["RydHQ_CargoOnly",[]]) + (_HQ getVariable ["RydHQ_Garrison",[]]));
							...
							if (not (_x in (_HQ getVariable ["RydHQ_NCCargoG",[]])) or ((count (units _x)) > 1)) then 
								...
								if (_ammo > 0) then
									...
									if not (_busy) then
										...
										[_x,_isAttacked,_HQ] spawn HAL_GoCapture;
										}
									}
								}
							}
