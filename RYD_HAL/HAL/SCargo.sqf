...
_CP = nearestObjects [_GL, ["Car","Tank","Motorcycle","Air","Ship"], (_HQ getVariable ["RydHQ_CargoFind",0])];
...
if (isNull _ChosenOne) then
	...
	_emptyV = false;
		...
		_ESpace = 0;
			...
			if ((((((leader _HQ) findNearestEnemy _GL) distance _GL) <= (500*_mpl)) or ((((leader _HQ) findNearestEnemy _halfway) distance _halfway) <= (500*_mpl))) and ((random 100) > (20*(0.5 + (2*(_HQ getVariable ["RydHQ_Recklessness",0.5])))))) then {_noenemy = false};
			if ((_ESpace >= _NG) and 
	...
	foreach ((_HQ getVariable ["RydHQ_CargoG",[]]) - ((_HQ getVariable ["RydHQ_NoCargo",[]]) + (_HQ getVariable ["RydHQ_SpecForG",[]])));
	};
