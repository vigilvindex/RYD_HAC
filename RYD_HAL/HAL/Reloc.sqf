private ["_HQ","_expression","_radius","_precision","_sourcesCount","_LU","_lastpos","_position","_Spot","_posX","_posY","_isWater"];
...
_sourcesCount = 1;
	...
	_lastpos = _x getvariable ("START" + (str _x));
	_position = [((position (vehicle (leader _HQ))) select 0) + (random 800) - 400,((position (vehicle (leader _HQ))) select 1) + (random 800) - 400];
	...
	if (not (_x in (_HQ getVariable ["RydHQ_AirG",[]])) and not
		(_iswater) and
			((_lastpos distance (_HQ getVariable ["RydHQ_Obj",(leader _HQ)])) > 2000) and 
