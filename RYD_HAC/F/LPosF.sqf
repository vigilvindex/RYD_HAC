waituntil {sleep 15; (RydHQF_Cyclecount > 2)};
...
while {not (isNull RydHQF)} do 
	...
	sleep 61;
		...
		if not (isPlayer (leader _x)) then 
			{
			if (not (_x in (RydHQF_RAirG + RydHQF_NCAirG)) and (_ammo == 0) and (((getposATL _veh) select 2) > 5)) then
				{
				...
				RydHQF_Exhausted = RydHQF_Exhausted + [_x];
				}
			else
				{
				if (RydHQF_LZ) then
					...
					if not (_x getVariable [("Resting" + (str _x)),false]) then
						...
						if not (_x getVariable [("Busy" + (str _x)),false]) then
							{
							_dst = (leader RydHQF) distance _start;
							if (_dst > RydHQF_AirDist) then
								{
								_newPos = [getPosATL (leader RydHQF),300] call RYD_RandomAround;
			...
			if (RydHQF_LZ) then
				...
				if (_dst > RydHQF_AirDist) then
					{
					_lz = [getPosATL (leader RydHQF)] call RYD_LZ;
					if not (isNull _lz) then
						{
						_start = getPosATL _lz;
						_x setVariable [("START" + str (_x)),_start]
