waitUntil {sleep 1; not (isNil "RydHQC_SpecForG")};

while {not (isNull RydHQC)} do
	{
	_pos = getPosATL (vehicle leaderHQC);
		...
		if not (_isBad) then
			...
			if not (_isWater) then 
				...
				_UL = leader _unitG;
				RydHQC_VCDone = true;
				if (isPlayer _UL) then {[_UL,leaderHQC] spawn VoiceComm;sleep 3;waituntil {sleep 0.1;(RydHQC_VCDone)}};
	...
	waitUntil
		...
		_nPos = getPosATL (vehicle leaderHQC);
