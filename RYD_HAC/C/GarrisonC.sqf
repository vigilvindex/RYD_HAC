...
waituntil {sleep 1; not (isNil "RydHQC_CargoAirEx")};
...
while {not (isNull RydHQC)} do
	{
	for [{_a = 0},{_a < (count RydHQC_Garrison)},{_a = _a + 1}] do
		{
		_unitG = RydHQC_Garrison select _a;
		...
		if not (_garrisoned) then
			...
			if ((RydHQC_Debug) or (isPlayer (leader _unitG))) then 
				{
				_i = [_pos,_unitG,"markGarrison","ColorBrown","ICON","mil_box","Garr C"," - GARRISON",[0.5,0.5]] call RYD_Mark;
				};

			if ((RydHQC_GarrVehAb) and not (isPlayer (leader _unitG))) then
				{
			...
			if (not (isNull _AV) and not (RydHQC_GarrVehAb)) exitWith
				{
				_frm = "DIAMOND";
