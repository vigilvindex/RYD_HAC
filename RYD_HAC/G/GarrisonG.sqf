...
waituntil {sleep 1; not (isNil "RydHQG_CargoAirEx")};
...
while {not (isNull RydHQG)} do
	{
	for [{_a = 0},{_a < (count RydHQG_Garrison)},{_a = _a + 1}] do
		{
		_unitG = RydHQG_Garrison select _a;
		...
		if not (_garrisoned) then
			...
			if ((RydHQG_Debug) or (isPlayer (leader _unitG))) then 
				{
				_i = [_pos,_unitG,"markGarrison","ColorBrown","ICON","mil_box","Garr G"," - GARRISON",[0.5,0.5]] call RYD_Mark;
				};

			if ((RydHQG_GarrVehAb) and not (isPlayer (leader _unitG))) then
				{
			...
			if (not (isNull _AV) and not (RydHQG_GarrVehAb)) exitWith
				{
				_frm = "DIAMOND";
