...
if ((RydHQG_Debug) or (isPlayer (leader _unitG))) then 
	{
	_i = [_DefPos,_unitG,"markDef","ColorRedAlpha","ICON","DOT","DRes G"," - DEFEND POSITION"] call RYD_Mark
	};
...
waituntil
	...
	if not (isNull _chosen) then 
		...
		if ((RydHQG_Debug) or (isPlayer (leader _unitG))) then 
			{
			_i = [_RnfP,_unitG,"markReinf","ColorRedAlpha","ICON","DOT","Reinf G"," - REINFORCE POSITION",[0.3,0.3]] call RYD_Mark
			}
		};
