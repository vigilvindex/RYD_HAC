waituntil {sleep 15; (RydHQG_Cyclecount > 2)};
...
while {not (isNull RydHQG)} do 
	...
	sleep 61;
		...
		if not (isPlayer (leader _x)) then 
			{
			if (not (_x in (RydHQG_RAirG + RydHQG_NCAirG)) and (_ammo == 0) and (((getposATL _veh) select 2) > 5)) then
				{
				...
				RydHQG_Exhausted = RydHQG_Exhausted + [_x];
				}
			else
				{
				if (RydHQG_LZ) then
					...
					if not (_x getVariable [("Resting" + (str _x)),false]) then
						...
						if not (_x getVariable [("Busy" + (str _x)),false]) then
							{
							_dst = (leader RydHQG) distance _start;
							if (_dst > RydHQG_AirDist) then
								{
								_newPos = [getPosATL (leader RydHQG),300] call RYD_RandomAround;
			...
			if (RydHQG_LZ) then
				...
				if (_dst > RydHQG_AirDist) then
					{
					_lz = [getPosATL (leader RydHQG)] call RYD_LZ;
					if not (isNull _lz) then
						{
						_start = getPosATL _lz;
						_x setVariable [("START" + str (_x)),_start]
