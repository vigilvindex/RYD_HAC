...
_CP = nearestObjects [_GL, ["Car","Tank","Motorcycle","Air","Ship"], RydHQG_CargoFind];
...
if (isNull _ChosenOne) then
	...
	_emptyV = false;
		...
		_ESpace = 0;
			...
			if ((group _x) in RydHQG_AirG) then {_mpl = 3} else {_mpl = 1};
			_noenemy = true;
			...
			if (((((leaderHQG findNearestEnemy _GL) distance _GL) <= (500*_mpl)) or (((leaderHQG findNearestEnemy _halfway) distance _halfway) <= (500*_mpl))) and ((random 100) > (20*(0.5 + (2*RydHQG_Recklessness))))) then {_noenemy = false};
			if ((_ESpace >= _NG) and 
				((count (assignedCargo (assignedvehicle _x))) == 0) and not 
					((_busy) or (_hired)) and not
						((_x in RydHQG_NCrewInfG) and (count (units _x) > 1)) and
								...
								((fuel (assignedvehicle _x)) >= 0.2) and 
									(damage (assignedvehicle _x) <= 0.8) and 
										(canMove (assignedvehicle _x)) and
											(_noenemy) and
												(not ((group _x) in RydHQG_AirG) or (((count (RydHQG_AAthreat + RydHQG_Airthreat)) == 0) or (random 100 > (85/(0.5 + (2*RydHQG_Recklessness))))))) exitwith 
				{
	...
	foreach (RydHQG_CargoG - (RydHQG_NoCargo + RydHQG_SpecForG));
	};
...
if not (_GD == _unitG) then 
	...
	if not (isNil ("_Vpos")) then {_LandPos = _GD getvariable ("START" + _unitvar)} else {_LandPos = [(position leaderHQG select 0) + (random 200) - 100,(position leaderHQG select 1) + (random 200) - 100]};
	sleep 5;
	if not (_GD in RydHQG_AirG) then {sleep 15};
