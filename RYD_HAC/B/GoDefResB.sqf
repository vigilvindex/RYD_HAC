...
if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then 
	{
	_i = [_DefPos,_unitG,"markDef","ColorWhite","ICON","mil_dot","DRes B"," - DEFEND POSITION"] call RYD_Mark
	};
...
waituntil
	...
	if not (isNull _chosen) then 
		...
		if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then 
			{
			_i = [_RnfP,_unitG,"markReinf","ColorWhite","ICON","mil_dot","Reinf B"," - REINFORCE POSITION",[0.3,0.3]] call RYD_Mark
			}
		};
