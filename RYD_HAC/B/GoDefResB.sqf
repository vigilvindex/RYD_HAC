...
if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then 
	{
	_i = [_DefPos,_unitG,"markDef","ColorRedAlpha","ICON","DOT","DRes B"," - DEFEND POSITION"] call RYD_Mark
	};
...
if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then 
	{
	_i = [_TED,_unitG,"markWatch","ColorGreenAlpha","ICON","DOT","B","B",[0.2,0.2]] call RYD_Mark
	};
...
waituntil
	...
	if not (isNull _chosen) then 
		...
		if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then 
			{
			_i = [_RnfP,_unitG,"markReinf","ColorRedAlpha","ICON","DOT","Reinf B"," - REINFORCE POSITION",[0.3,0.3]] call RYD_Mark
			}
		};
