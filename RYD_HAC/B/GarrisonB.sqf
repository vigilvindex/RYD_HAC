...
waituntil {sleep 1; not (isNil "RydHQB_CargoAirEx")};
...
while {not (isNull RydHQB)} do
	{
	for [{_a = 0},{_a < (count RydHQB_Garrison)},{_a = _a + 1}] do
		{
		_unitG = RydHQB_Garrison select _a;
		...
		if not (_garrisoned) then
			...
			if ((RydHQB_Debug) or (isPlayer (leader _unitG))) then 
				{
				_i = [_pos,_unitG,"markGarrison","ColorBrown","ICON","mil_box","Garr B"," - GARRISON",[0.5,0.5]] call RYD_Mark;
				};

			if ((RydHQB_GarrVehAb) and not (isPlayer (leader _unitG))) then
				{
			...
			if (not (isNull _AV) and not (RydHQB_GarrVehAb)) exitWith
				{
				_frm = "DIAMOND";
