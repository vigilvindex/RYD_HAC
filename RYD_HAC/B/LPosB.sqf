waituntil {sleep 15; (RydHQB_Cyclecount > 2)};
...
while {not (isNull RydHQB)} do 
	...
	sleep 61;
		...
		if not (isPlayer (leader _x)) then 
			{
			if (not (_x in (RydHQB_RAirG + RydHQB_NCAirG)) and (_ammo == 0) and (((getposATL _veh) select 2) > 5)) then
				{
				...
				RydHQB_Exhausted = RydHQB_Exhausted + [_x];
				}
			else
				{
				if (RydHQB_LZ) then
					...
					if not (_x getVariable [("Resting" + (str _x)),false]) then
						...
						if not (_x getVariable [("Busy" + (str _x)),false]) then
							{
							_dst = (leader RydHQB) distance _start;
							if (_dst > RydHQB_AirDist) then
								{
								_newPos = [getPosATL (leader RydHQB),300] call RYD_RandomAround;
			...
			if (RydHQB_LZ) then
				...
				if (_dst > RydHQB_AirDist) then
					{
					_lz = [getPosATL (leader RydHQB)] call RYD_LZ;
					if not (isNull _lz) then
						{
						_start = getPosATL _lz;
						_x setVariable [("START" + str (_x)),_start]
