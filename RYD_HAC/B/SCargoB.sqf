...
_CP = nearestObjects [_GL, ["Car","Tank","Motorcycle","Air","Ship"], RydHQB_CargoFind];
...
if (isNull _ChosenOne) then
	...
	_emptyV = false;
		...
		_ESpace = 0;
			...
			if ((group _x) in RydHQB_AirG) then {_mpl = 3} else {_mpl = 1};
			_noenemy = true;
			...
			if (((((leaderHQB findNearestEnemy _GL) distance _GL) <= (500*_mpl)) or (((leaderHQB findNearestEnemy _halfway) distance _halfway) <= (500*_mpl))) and ((random 100) > (20*(0.5 + (2*RydHQB_Recklessness))))) then {_noenemy = false};
			if ((_ESpace >= _NG) and 
				((count (assignedCargo (assignedvehicle _x))) == 0) and not 
					((_busy) or (_hired)) and not
						((_x in RydHQB_NCrewInfG) and (count (units _x) > 1)) and
								...
								((fuel (assignedvehicle _x)) >= 0.2) and 
									(damage (assignedvehicle _x) <= 0.8) and 
										(canMove (assignedvehicle _x)) and
											(_noenemy) and
												(not ((group _x) in RydHQB_AirG) or (((count (RydHQB_AAthreat + RydHQB_Airthreat)) == 0) or (random 100 > (85/(0.5 + (2*RydHQB_Recklessness))))))) exitwith 
				{
	...
	foreach (RydHQB_CargoG - (RydHQB_NoCargo + RydHQB_SpecForG));
	};
...
if not (_GD == _unitG) then 
	...
	if not (isNil ("_Vpos")) then {_LandPos = _GD getvariable ("START" + _unitvar)} else {_LandPos = [(position leaderHQB select 0) + (random 200) - 100,(position leaderHQB select 1) + (random 200) - 100]};
	sleep 5;
	if not (_GD in RydHQB_AirG) then {sleep 15};
