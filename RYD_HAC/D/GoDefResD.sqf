...
if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then 
	{
	_i = [_DefPos,_unitG,"markDef","ColorWhite","ICON","mil_dot","DRes D"," - DEFEND POSITION"] call RYD_Mark
	};
...
waituntil
	...
	if not (isNull _chosen) then 
		...
		if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then 
			{
			_i = [_RnfP,_unitG,"markReinf","ColorWhite","ICON","mil_dot","Reinf D"," - REINFORCE POSITION",[0.3,0.3]] call RYD_Mark
			}
		};
