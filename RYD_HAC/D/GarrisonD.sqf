...
waituntil {sleep 1; not (isNil "RydHQD_CargoAirEx")};
...
while {not (isNull RydHQD)} do
	{
	for [{_a = 0},{_a < (count RydHQD_Garrison)},{_a = _a + 1}] do
		{
		_unitG = RydHQD_Garrison select _a;
		...
		if not (_garrisoned) then
			...
			if ((RydHQD_Debug) or (isPlayer (leader _unitG))) then 
				{
				_i = [_pos,_unitG,"markGarrison","ColorBrown","ICON","mil_box","Garr D"," - GARRISON",[0.5,0.5]] call RYD_Mark;
				};

			if ((RydHQD_GarrVehAb) and not (isPlayer (leader _unitG))) then
				{
			...
			if (not (isNull _AV) and not (RydHQD_GarrVehAb)) exitWith
				{
				_frm = "DIAMOND";
