...
waituntil {sleep 1; not (isNil "RydHQE_CargoAirEx")};
...
while {not (isNull RydHQE)} do
	{
	for [{_a = 0},{_a < (count RydHQE_Garrison)},{_a = _a + 1}] do
		{
		_unitG = RydHQE_Garrison select _a;
		...
		if not (_garrisoned) then
			...
			if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then 
				{
				_i = [_pos,_unitG,"markGarrison","ColorBrown","ICON","mil_box","Garr E"," - GARRISON",[0.5,0.5]] call RYD_Mark;
				};

			if ((RydHQE_GarrVehAb) and not (isPlayer (leader _unitG))) then
				{
			...
			if (not (isNull _AV) and not (RydHQE_GarrVehAb)) exitWith
				{
				_frm = "DIAMOND";
