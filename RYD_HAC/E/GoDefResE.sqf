...
if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then 
	{
	_i = [_DefPos,_unitG,"markDef","ColorWhite","ICON","mil_dot","DRes E"," - DEFEND POSITION"] call RYD_Mark
	};
...
if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then 
	{
	_i = [_TED,_unitG,"markWatch","Default","ICON","mil_dot","E","E",[0.2,0.2]] call RYD_Mark
	};
...
waituntil
	...
	if not (isNull _chosen) then 
		...
		if ((RydHQE_Debug) or (isPlayer (leader _unitG))) then 
			{
			_i = [_RnfP,_unitG,"markReinf","ColorWhite","ICON","mil_dot","Reinf E"," - REINFORCE POSITION",[0.3,0.3]] call RYD_Mark
			}
		};
