waituntil {sleep 15; (RydHQE_Cyclecount > 2)};
...
while {not (isNull RydHQE)} do 
	...
	sleep 61;
		...
		if not (isPlayer (leader _x)) then 
			{
			if (not (_x in (RydHQE_RAirG + RydHQE_NCAirG)) and (_ammo == 0) and (((getposATL _veh) select 2) > 5)) then
				{
				...
				RydHQE_Exhausted = RydHQE_Exhausted + [_x];
				}
			else
				{
				if (RydHQE_LZ) then
					...
					if not (_x getVariable [("Resting" + (str _x)),false]) then
						...
						if not (_x getVariable [("Busy" + (str _x)),false]) then
							{
							_dst = (leader RydHQE) distance _start;
							if (_dst > RydHQE_AirDist) then
								{
								_newPos = [getPosATL (leader RydHQE),300] call RYD_RandomAround;
			...
			if (RydHQE_LZ) then
				...
				if (_dst > RydHQE_AirDist) then
					{
					_lz = [getPosATL (leader RydHQE)] call RYD_LZ;
					if not (isNull _lz) then
						{
						_start = getPosATL _lz;
						_x setVariable [("START" + str (_x)),_start]
