waitUntil {sleep 1; not (isNil "RydHQH_SpecForG")};

while {not (isNull RydHQH)} do
	{
	_pos = getPosATL (vehicle leaderHQH);
		...
		if not (_isBad) then
			...
			if not (_isWater) then 
				...
				_UL = leader _unitG;
				RydHQH_VCDone = true;
				if (isPlayer _UL) then {[_UL,leaderHQH] spawn VoiceComm;sleep 3;waituntil {sleep 0.1;(RydHQH_VCDone)}};
	...
	waitUntil
		...
		_nPos = getPosATL (vehicle leaderHQH);
