waituntil {sleep 15; (RydHQH_Cyclecount > 2)};
...
while {not (isNull RydHQH)} do 
	...
	sleep 61;
		...
		if not (isPlayer (leader _x)) then 
			{
			if (not (_x in (RydHQH_RAirG + RydHQH_NCAirG)) and (_ammo == 0) and (((getposATL _veh) select 2) > 5)) then
				{
				...
				RydHQH_Exhausted = RydHQH_Exhausted + [_x];
				}
			else
				{
				if (RydHQH_LZ) then
					...
					if not (_x getVariable [("Resting" + (str _x)),false]) then
						...
						if not (_x getVariable [("Busy" + (str _x)),false]) then
							{
							_dst = (leader RydHQH) distance _start;
							if (_dst > RydHQH_AirDist) then
								{
								_newPos = [getPosATL (leader RydHQH),300] call RYD_RandomAround;
			...
			if (RydHQH_LZ) then
				...
				if (_dst > RydHQH_AirDist) then
					{
					_lz = [getPosATL (leader RydHQH)] call RYD_LZ;
					if not (isNull _lz) then
						{
						_start = getPosATL _lz;
						_x setVariable [("START" + str (_x)),_start]
