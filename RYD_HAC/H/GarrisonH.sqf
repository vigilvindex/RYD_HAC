...
waituntil {sleep 1; not (isNil "RydHQH_CargoAirEx")};
...
while {not (isNull RydHQH)} do
	{
	for [{_a = 0},{_a < (count RydHQH_Garrison)},{_a = _a + 1}] do
		{
		_unitG = RydHQH_Garrison select _a;
		...
		if not (_garrisoned) then
			...
			if ((RydHQH_Debug) or (isPlayer (leader _unitG))) then 
				{
				_i = [_pos,_unitG,"markGarrison","ColorBrown","ICON","mil_box","Garr H"," - GARRISON",[0.5,0.5]] call RYD_Mark;
				};

			if ((RydHQH_GarrVehAb) and not (isPlayer (leader _unitG))) then
				{
			...
			if (not (isNull _AV) and not (RydHQH_GarrVehAb)) exitWith
				{
				_frm = "DIAMOND";
