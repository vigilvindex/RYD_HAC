...
waituntil {sleep 1; not (isNil "RydHQ_CargoAirEx")};
...
while {not (isNull RydHQ)} do
	{
	for [{_a = 0},{_a < (count RydHQ_Garrison)},{_a = _a + 1}] do
		{
		_unitG = RydHQ_Garrison select _a;
		...
		if not (_garrisoned) then
			...
			if ((RydHQ_Debug) or (isPlayer (leader _unitG))) then 
				{
				_i = [_pos,_unitG,"markGarrison","ColorBrown","ICON","mil_box","Garr A"," - GARRISON",[0.5,0.5]] call RYD_Mark;
				};

			if ((RydHQ_GarrVehAb) and not (isPlayer (leader _unitG))) then
				{
			...
			if (not (isNull _AV) and not (RydHQ_GarrVehAb)) exitWith
				{
				_frm = "DIAMOND";
