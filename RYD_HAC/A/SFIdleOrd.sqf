waitUntil {sleep 1; not (isNil "RydHQ_SpecForG")};

while {not (isNull RydHQ)} do
	{
	_pos = getPosATL (vehicle leaderHQ);
		...
		if not (_isBad) then
			...
			if not (_isWater) then 
				...
				_UL = leader _unitG;
				RydHQ_VCDone = true;
				if (isPlayer _UL) then {[_UL,leaderHQ] spawn VoiceComm;sleep 3;waituntil {sleep 0.1;(RydHQ_VCDone)}};
	...
	waitUntil
		...
		_nPos = getPosATL (vehicle leaderHQ);
